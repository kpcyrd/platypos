            ...
            .zip(
                self.sha256sums
                    ...
                    .chain(
                        self.sha512sums
                            .iter()
                            .map(|sum| format!("sha512:{sum}"))
                            .chain(self.b2sums.iter().map(|sum| format!("b2:{sum}"))),
                    ),
            )
            .map(|(source, chksum)| {
                let (_, source) = source.split_once("::").unwrap_or(("", source));
                Source {
                    chksum,
...
pub fn parse(srcinfo: &str) -> Result<Vec<Pkg>> {
    ...
    loop {
        ...
        if let Some(value) = line.strip_prefix("pkgname = ") {
        ...
        } else if let Some(value) = line.strip_prefix("\tpkgdesc = ") {
            pkg.pkgdesc = value.to_string();
            ...
            pkg.url = value.to_string();
        } else if let Some(value) = line.strip_prefix("\tarch = ") {
            pkg.arch = value.to_string();
        } else if let Some(value) = line.strip_prefix("\tdepends = ") {
            pkg.depends.push(value.to_string());
        } else if let Some(value) = line.strip_prefix("\tmakedepends = ") {
            pkg.makedepends.push(value.to_string());
        } else if let Some(value) = line.strip_prefix("\tsource = ") {
            pkg.source.push(value.to_string());
        } else if let Some(value) = line.strip_prefix("\tsha256sums = ") {
            pkg.sha256sums.push(value.to_string());
        } else if let Some(value) = line.strip_prefix("\tsha512sums = ") {
            pkg.sha512sums.push(value.to_string());
        } else if let Some(value) = line.strip_prefix("\tb2sums = ") {
            pkg.b2sums.push(value.to_string());
        }
...
mod tests {
    ...
    fn test_parse_simple() {
        ...
        assert_eq!(
            ...
            vec![Pkg {
                ...
                pkgdesc: "Cross-platform Rust rewrite of the GNU coreutils".to_string(),
                pkgver: "0.1.0".to_string(),
                pkgrel: "1".to_string(),
                epoch: None,
                url: "https://uutils.github.io/".to_string(),
                ...
                makedepends: vec![
                    "cargo".to_string(),
                    "clang".to_string(),
                    "python-sphinx".to_string(),
                    "rust".to_string(),
                ],
                source: vec![
                    "coreutils-uutils-0.1.0.tar.gz::https://github.com/uutils/coreutils/archive/0.1.0.tar.gz".to_string(),
                    "disable_selinux.patch".to_string(),
                ],
                sha256sums: vec![],
                sha512sums: vec![
                    "e9b3941573b2267e6a4b17c49a64d71234e4cbf067f1180fe14bfe48012c9d400741067f6231855a6132f93138eb4bbcb1e970e98c5f937f8d3a5e5bc019f03c".to_string(),
                    "c1585671d2e0fe34a92746eae291f1be85e65c7ccfe2462cba5d324b8def76296f71b6d15738ecb2683453886de9c0650a0b9a609cad5f03b6ded89e5a91911c".to_string(),
                ],
                b2sums: vec![],
            }]
        );
    ...
    // in the long run we likely read those fields from .PKGINFO anyway
    fn clear_some_data(mut pkgs: Vec<Pkg>) -> Vec<Pkg> {
        for pkg in &mut pkgs {
            pkg.depends.clear();
            pkg.makedepends.clear();
            pkg.source.clear();
            pkg.sha256sums.clear();
            pkg.sha512sums.clear();
            pkg.b2sums.clear();
        }
        pkgs
    }
    ...
    fn test_parse_advanced() {
        ...
        let pkgs = clear_some_data(parse(srcinfo).unwrap());
        assert_eq!(
            pkgs,
            vec![
                Pkg {
                    pkgname: "systemd".to_string(),
                    pkgdesc: "system and service manager".to_string(),
                    pkgver: "257.6".to_string(),
                    pkgrel: "1".to_string(),
                    epoch: None,
                    url: "https://www.github.com/systemd/systemd".to_string(),
                    arch: "x86_64".to_string(),
                    depends: vec![],
                    makedepends: vec![],
                    source: vec![],
                ...
                Pkg {
                    ...
                    pkgdesc: "systemd client libraries".to_string(),
                    pkgver: "257.6".to_string(),
                    pkgrel: "1".to_string(),
                    epoch: None,
                    url: "https://www.github.com/systemd/systemd".to_string(),
                ...
                Pkg {
                    ...
                    pkgdesc: "sysvinit compat for systemd".to_string(),
                    pkgver: "257.6".to_string(),
                    pkgrel: "1".to_string(),
                    epoch: None,
                    url: "https://www.github.com/systemd/systemd".to_string(),
                ...
                Pkg {
                    ...
                    pkgdesc: "systemd tests".to_string(),
                    pkgver: "257.6".to_string(),
                    pkgrel: "1".to_string(),
                    epoch: None,
                    url: "https://www.github.com/systemd/systemd".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ...
                Pkg {
                    ...
                    pkgdesc: "32-bit target and libraries for Rust".to_string(),
                    pkgver: "1.87.0".to_string(),
                    pkgrel: "1".to_string(),
                    epoch: Some("1".to_string()),
                    url: "https://www.rust-lang.org/".to_string(),
                ...
                Pkg {
                    ...
                    pkgdesc: "Musl target for Rust".to_string(),
                    pkgver: "1.87.0".to_string(),
                    pkgrel: "1".to_string(),
                    epoch: Some("1".to_string()),
                    url: "https://www.rust-lang.org/".to_string(),
                ...
                Pkg {
                    ...
                    pkgdesc: "Source code for the Rust standard library".to_string(),
                    pkgver: "1.87.0".to_string(),
                    pkgrel: "1".to_string(),
                    epoch: Some("1".to_string()),
                    url: "https://www.rust-lang.org/".to_string(),
